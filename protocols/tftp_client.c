...
static void tftp_ensure_help_file(Storage* storage) {
    ...
    if(storage_file_open(file, TFTP_HELP_PATH, FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        static const char help[] =
            "PXE Download & TFTP — LAN Tester for Flipper Zero\n"
            "\n"
            "== PXE Boot Files ==\n"
            "\n"
            "PXE Download (Utilities menu) fetches common\n"
            "boot files from the network and saves them to:\n"
            ...
            "   .0     — PXELinux\n"
            "\n"
            "Use these files with your own PXE/DHCP\n"
            "infrastructure — this app does not include a\n"
            "PXE/TFTP server, only a downloader and a\n"
            "generic TFTP client (below).\n"
            "\n"
            "== Target BIOS/UEFI ==\n"
            "\n"
